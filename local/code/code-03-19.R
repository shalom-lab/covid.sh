...
# VARIABLE
v.date<-'2022-03-19'
v.url.case<-'https://mp.weixin.qq.com/s/ZkFbmJMo6tUiFHezQeArsQ'
v.url.location<-'https://mp.weixin.qq.com/s/njMjbpRELpe7SWA7AnB5NA'
...
df.case.1$group<-mf.tag(c('isolation','screen','asym'),c(1,10,12),17)
df.asym.1$group<-mf.tag(c('isolation','screen'),c(1,234),492)
...
df.map.1<-html.location %>% html_elements('section > p > span') %>% html_text() %>% 
  as_tibble() %>%
  rownames_to_column(var = 'id') %>%
  mutate(id=as.integer(id),
         wordcount=str_count(value),
         districtIndex=str_detect(value,'^2022年')) %>%
  arrange(-wordcount)
