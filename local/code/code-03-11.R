...
# VARIABLE
v.date<-'2022-03-11'
v.url <-'https://mp.weixin.qq.com/s/e_1M17uJPRITsLaYcdgnxA'
...
df.case.1$group<-mf.tag(c('isolation','screen'),c(1,5),5)
df.asym.1$group<-mf.tag(c('isolation','screen'),c(1,58),77)

df.asym.1 <- df.asym.1 %>%
  add_row(gender='男',age=59,district='浦东新区',group='isolation')
