...
# VARIABLE
v.date<-'2022-03-15'
v.url <-'https://mp.weixin.qq.com/s/G77OEw8GgiJQiAkRH3tOlw'
...
df.case.1<-data.frame(text=html.case %>% html_elements('p') %>% html_text()) %>%
  ...
  mutate(gender=t2,
         ...
         district=str_extract(str_replace_all(address,'小区|生活区|工业园区|世博家园A区','XIAOQU'),'.+区'),
         date=ymd(v.date))
...
df.case.1$group<-mf.tag(c('isolation','screen','asym'),c(1,4,5),5)
df.asym.1$group<-mf.tag(c('isolation','screen','isolation','screen'),c(1,67,102,171),196)
