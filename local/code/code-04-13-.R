...
df.url<-df.url %>%
  bind_rows(c(v.date='2022-06-14',
            v.url.case='https://mp.weixin.qq.com/s/ohLs8WWPWJBkuaL4iYXrhw',
            v.url.location='https://mp.weixin.qq.com/s/jNlwZj7s19emjB58lE9rzg')) %>%
  distinct(v.date,.keep_all = T) %>%
  arrange(v.date)
saveRDS(df.url,'data/df.url.RDS')

tem.df<-filter(df.url,v.date=='2022-06-14')
...
# fill group
df.case.1$group<-mf.tag(c('isolation','screen','asym'),c(1,3,5),dim(df.case.1)[1])
df.asym.1$group<-mf.tag(c('isolation','screen'),c(1,2),dim(df.asym.1)[1])
