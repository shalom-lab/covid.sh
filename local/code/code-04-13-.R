...
df.url<-df.url %>%
  bind_rows(c(v.date='2022-06-05',
            v.url.case='https://mp.weixin.qq.com/s/44X2n4RQSLpC9FLU2MThwQ',
            v.url.location='https://mp.weixin.qq.com/s/zERWgFNJzWTydSmvjRPFLw')) %>%
  distinct(v.date,.keep_all = T) %>%
  arrange(v.date)
saveRDS(df.url,'data/df.url.RDS')

tem.df<-filter(df.url,v.date=='2022-06-05')
