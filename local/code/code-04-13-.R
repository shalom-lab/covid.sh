...
df.url<-df.url %>%
  bind_rows(c(v.date='2022-06-06',
            v.url.case='https://mp.weixin.qq.com/s/tC4KmWQ5r8eqfeTav5xAeQ',
            v.url.location='https://mp.weixin.qq.com/s/ft2t7EVyT96YmwXfvd-zQg')) %>%
  distinct(v.date,.keep_all = T) %>%
  arrange(v.date)
saveRDS(df.url,'data/df.url.RDS')

tem.df<-filter(df.url,v.date=='2022-06-06')
...
# fill group
df.case.1$group<-mf.tag(c('screen'),c(1),dim(df.case.1)[1])
