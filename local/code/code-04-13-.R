...
df.url<-df.url %>%
  bind_rows(c(v.date='2022-04-21',
            v.url.case='https://mp.weixin.qq.com/s/BTtYkDdU6t6OGF0a8kE3Zg',
            v.url.location='https://mp.weixin.qq.com/s/qFvUyEB-R-GKP7vgKR-c3A')) %>%
...
# fill group
df.case.1$group<-mf.tag(c('isolation','screen','asym'),c(1,17,29),41)
df.asym.1$group<-mf.tag(c('isolation','screen'),c(1,17),29)
