...
df.url<-df.url %>%
  bind_rows(c(v.date='2022-05-21',
            v.url.case='https://mp.weixin.qq.com/s/q385QELBASHyzn1xVYVNlA',
            v.url.location='https://mp.weixin.qq.com/s/XWA8Pzf0DQ5i92si5Epsgg')) %>%
  distinct(v.date,.keep_all = T) %>%
  arrange(v.date)
saveRDS(df.url,'data/df.url.RDS')

tem.df<-filter(df.url,v.date=='2022-05-21')
...
df.case.1<-data.frame(text=html.case %>% html_elements(c('p','section')) %>% html_text()) %>%
...
df.asym.1<-data.frame(text=html.case %>% html_elements(c('p','section')) %>% html_text()) %>%
...
# fill group
df.case.1$group<-mf.tag(c('isolation','asym'),c(1,8),11)
df.asym.1$group<-mf.tag(c('isolation'),c(1),12)
