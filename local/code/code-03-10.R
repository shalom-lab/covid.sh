...
# VARIABLE
v.date<-'2022-03-10'
v.url <-'https://mp.weixin.qq.com/s/TXbtjAgxlR5rvgaCJ_dt7A'
...
df.case.1$group<-mf.tag(c('isolation','screen'),c(1,5),10)
df.asym.1$group<-mf.tag(c('isolation','screen','isolation','screen'),c(1,21,22,63),64)

df.case.1 <- df.case.1 %>%
  add_row(gender='男',age=39,district='浦东新区',group='isolation')
