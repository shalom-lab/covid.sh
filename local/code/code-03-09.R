...
# VARIABLE
v.date<-'2022-03-09'
v.url <-'https://mp.weixin.qq.com/s/9_HUJ4bc58bJP_InkqMKhw'
...
df.case.1$group<-mf.tag(c('isolation','screen'),c(1,3),3)
df.asym.1$group<-mf.tag(c('isolation','screen'),c(1,66),76)

df.case.1 <- df.case.1 %>%
  add_row(gender='女',age=31,district='虹口区',group='isolation',date=as.Date('2022-03-09'))

df.case.1[3,'district']<-'浦东新区'
